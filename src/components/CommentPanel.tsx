...
/**
 * 模組名稱：評論面板 (CommentPanel.tsx)
 * 功能描述：影片下方的互動評論區。支持實時留言顯示、發送功能及鍵盤彈出適配。
 */
...
export default function CommentPanel({ storeName }: CommentPanelProps) {
  const { showComments, setShowComments } = useEmpireStore();
        ...
        <>
          ...
          />
            <motion.div 
              ...
              style={{ 
                paddingBottom: isInputFocused ? `${keyboardHeight || 336}px` : '0px'
              }}
              className="fixed left-0 right-0 bottom-0 bg-zinc-900 border-t border-gold-primary/30 rounded-t-[2.5rem] z-[1001] flex flex-col overflow-hidden"
            >
            <div className="p-8 space-y-6 flex-shrink-0">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setShowComments(false)} className="p-2 rounded-full bg-white/5 text-gold-primary"><X size={20} /></button>
              </div>

...
/**
 * 模組名稱：懸賞面板 (BountyPanel.tsx)
 * 功能描述：用戶發布懸賞任務的介面。允許設定賞金金額、任務描述及截止時間。
 */
...
export default function BountyPanel({ onConfirm }: BountyPanelProps) {
  const { showBounty, setShowBounty } = useEmpireStore();
      ...
      {showBounty && (
        <>
          <motion.div 
            ...
            className="fixed inset-0 bg-black/30 backdrop-blur-sm z-[1000] pointer-events-auto"
          />
          <motion.div 
            ...
            className="fixed inset-0 flex items-center justify-center p-6 z-[1001] pointer-events-auto"
          >
